...
pub mod staking_app {
    ...
    pub fn stake_token(_ctx: Context<StakeToken>, _amount: u64, _is_stake: bool) -> Result<()> {
        Ok(())
    }
}
...
pub struct StakeToken<'info>{
    ...
    /// CHECK: Chỉ là PDA dùng để ký tên
    #[account(seeds = [b"TOKEN_VAULT_AUTH"], bump)]
    pub vault_authority: UncheckedAccount<'info>,
    
    #[account(
        init_if_needed,
        payer = payer,
        seeds = [b"TOKEN_VAULT_AUTH", mint.key().as_ref()],
        bump,
        token::mint = mint,
    ...
    #[account(
        init_if_needed,
        seeds = [b"USER_TOKEN_INFO", user.key().as_ref(), mint.key().as_ref()],
        bump,
        payer = payer,
        space = 8 + std::mem::size_of::<UserInfo>(),
    )]
    pub user_token_info: Box<Account<'info, UserInfo>>,
